...
func (d *DB) CreateWorkspace(ctx context.Context, create *store.Workspace) (*store.Workspace, error) {
	...
	stmt := "INSERT INTO workspace (" + strings.Join(fields, ", ") + ") VALUES (" + placeholders(len(args)) + ") RETURNING id, created_ts, updated_ts, sort_field, sort_order, cover_color, cover_image, folders_first, display_order, hidden"
	if err := d.db.QueryRowContext(ctx, stmt, args...).Scan(&create.ID, &create.CreatedTs, &create.UpdatedTs, &create.SortField, &create.SortOrder, &create.CoverColor, &create.CoverImage, &create.FoldersFirst, &create.DisplayOrder, &create.Hidden); err != nil {
		return nil, err
	}
	return create, nil
}
...
func (d *DB) ListWorkspaces(ctx context.Context, find *store.FindWorkspace) ([]*store.Workspace, error) {
	...
	if v := find.StorageSlug; v != nil {
		where, args = append(where, "storage_slug = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := find.Hidden; v != nil {
		where, args = append(where, "hidden = "+placeholder(len(args)+1)), append(args, *v)
	}
	...
	rows, err := d.db.QueryContext(ctx, `
		SELECT id, uid, creator_id, title, created_ts, updated_ts, sort_field, sort_order, cover_color, cover_image, folders_first, storage_slug, display_order, hidden
		FROM workspace
...
func (d *DB) UpdateWorkspace(ctx context.Context, update *store.UpdateWorkspace) (*store.Workspace, error) {
	...
	if v := update.StorageSlug; v != nil {
		set, args = append(set, "storage_slug = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := update.DisplayOrder; v != nil {
		set, args = append(set, "display_order = "+placeholder(len(args)+1)), append(args, *v)
	}
	if v := update.Hidden; v != nil {
		set, args = append(set, "hidden = "+placeholder(len(args)+1)), append(args, *v)
	}
	...
	stmt := `
		...
		RETURNING id, uid, creator_id, title, created_ts, updated_ts, sort_field, sort_order, cover_color, cover_image, folders_first, storage_slug, display_order, hidden
	`
	args = append(args, update.ID)

	w := &store.Workspace{}
	if err := d.db.QueryRowContext(ctx, stmt, args...).Scan(&w.ID, &w.UID, &w.CreatorID, &w.Title, &w.CreatedTs, &w.UpdatedTs, &w.SortField, &w.SortOrder, &w.CoverColor, &w.CoverImage, &w.FoldersFirst, &w.StorageSlug, &w.DisplayOrder, &w.Hidden); err != nil {
		return nil, err
	}
	return w, nil
}
