      ...
      <div className="flex w-full shrink flex-col items-start justify-start gap-1.5 overflow-y-auto overflow-x-hidden p-0.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {primaryNavLinks.map((navLink) => (
          <NavLink
            ...
            viewTransition
          >
            <div className="shrink-0" data-sidebar-expand-trigger>
              {navLink.icon}
            </div>
            <span
              className={cn(
                "truncate whitespace-nowrap font-medium transition-[max-width,margin,opacity] duration-200",
                props.collapsed ? "ml-0 max-w-0 opacity-0" : isMini ? "ml-2.5 max-w-40 opacity-100" : "ml-3 max-w-44 opacity-100",
              )}
