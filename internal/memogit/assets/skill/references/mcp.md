...
| `attachment_get_download_url` | 把一个附件换成短期下载链接 |
| `auth_get_current_user` | 令牌属于谁 |

**没有删除工具，这是刻意的。** 评论、reaction、关系、快捷方式也都不暴露——
需要它们请走 memogit 或让用户在网页端处理。附件只开了一个只读的下载口子，见 §7。
...
- **文件夹是隐式的**：写到一个不存在的路径，文件夹自己出现在树上。没有建文件夹这一步，
  工具集里也没有文件夹工具。
- **标题不带扩展名**：传 `plan.md` 会得到一篇字面叫 "plan.md" 的文档
  （见 `hierarchy-and-doc-types.md` §3）。
...
4. **定制语法照样适用。** 通过 MCP 写入的内容和 memogit 写入的是同一批文档：
   正文默认只写基本 Markdown（callout / `==着色==` / 点击计数器这些装饰语法一概不写，
   见铁律 3）、frontmatter 只认扁平字段、标题里别放标点、`toucan-secret` 的 `id` 别动、
   交互块只在用户明确要求时写且写回契约照旧。见 `markdown-syntax.md` 和
   `blocks-and-views.md`。
...
- **没有评论、关系**：在网页端或 memogit 检出里看。
- **附件只能读，而且要用户先点名**：见下面 §7。

## 7. 附件：用户点名，你去下载

MCP 这边**没有列附件的工具**——找哪个附件是用户的事，不是你的事。
（`memo_get_memo` 的返回里确实带 `attachments` 数组，能看到文件名和类型，但没有字节。）

典型流程是用户在 app 里点附件行的 **⋮ → 复制 Markdown 引用**，把
`![spec.pdf](/file/attachments/abc123/spec.pdf)` 贴给你说"读这个"。

拿到之后：

1. **`attachment_get_download_url`**，参数就把用户给你的东西**原样贴进去**。
   资源名 `attachments/{uid}`、`/file/...` 路径、带域名的完整 URL、整段 markdown 引用，
   都认得，不用你先解析。
2. 返回里有 `url`、`filename`、`type`、`size`、`expire_time`。
   **链接是短期的（几分钟），拿到就用。**
3. **用你自己的下载能力把它存成本地文件，然后读那个文件。**
   别试图把字节塞进对话——链接存在的意义就是绕开这件事。

几条边界：

- **只服务能读的格式**：纯文本、PDF、png/jpeg/gif/webp、SVG。
  Office 文件、音视频、EPUB、压缩包一律拒绝，**告诉用户在 app 里自己处理**，别绕。
- **保险柜锁定的附件拒绝**，因为解锁是浏览器会话的事。你会拿到一句明确的提示，转述给用户即可。
- **权限没有任何放宽**：签发时按你的身份查一遍，下载时再查一遍。
  你读不了的文档，它的附件你也拿不到。
- **只读**。没有上传、替换、删除附件的工具，以后也不会有。用户要改附件，让他在 app 里做。

拿到文件之后怎么判断该不该细读、以及"没读的要主动说出来"，
和 memogit 那边是同一套规矩，见 `attachments.md` §3。
