...
③  改数据库指针            （reference / payload.key，并删除旧 payload.s3_config）
```
...
**源在哪**：新附件只保存 key，默认使用实例当前配置。真正换 endpoint/bucket 时，
运维通过 `--source-endpoint` / `--source-bucket` 一次性提供旧位置。升级前已写入
`payload.s3_object.s3_config` 的旧附件仍可用其快照完成最后一次迁移；显式 CLI 参数
优先于旧快照。
...
- 删除遗留的 `payload.s3_object.s3_config`

S3 连接信息是实例级配置，不是附件身份的一部分。附件只持有 key，读取、删除、
缩略图与备份均从实例 `STORAGE` 设置解析 endpoint/bucket/凭证。这样 endpoint 改名
只改一处，不会把域名和凭证复制到每一行附件中。
...
| 源 endpoint 已下线 | 换了云厂商且旧厂商已注销 | 源读不到，条目失败并记录；这种情况下附件本来就已经丢了，迁移只是把事实暴露出来 |
