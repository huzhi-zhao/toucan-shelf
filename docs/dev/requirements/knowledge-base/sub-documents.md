...
和附件区块一样在大纲里有一个跳转入口，与
`ATTACHMENTS_ANCHOR_ID`（[DocumentOutline.tsx:11](../../../../web/src/components/Notebook/DocumentOutline.tsx)）
同构。
...
- **发现路径**：`memo_get_memo(父文档)` 已经返回含 COMMENT 类型的 relations
  （[memo_service_converter.go:310](../../../../server/router/api/v1/memo_service_converter.go)），
  但那是一串裸关系。需要给列表接口加一个"列出某父文档的子文档"的过滤，
  并把约定写进 [`docs/skill/`](../../../skill/)。
...
- **pull** 需要服务端能列出子文档，就是 §5 提到的那个列表过滤。
