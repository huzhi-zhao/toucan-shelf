...
来源：写回能力不是后来补的，它和块本身同一天落地——`c89821ef`（2026-07-12，Calendar
Block）建块，`e1f0cbf8`（同日，"能直接操作 md 文档 在预览界面"）加上写回。之后
`2d08669c`（2026-07-19）扩了 Events，`e28b43db`（2026-07-21）把 md 块统一到同一套渲染
组件。**没有独立的需求文档**，全部线索只有这四个 commit，本篇就是它的第一份需求记录。
...
菜单项的文案 key 是 `editor.view.calendar`。
