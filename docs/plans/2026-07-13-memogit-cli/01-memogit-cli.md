...
> **2026-07-28 修订：uid 移回文件（其余元数据不变）**
>
> 本节"元数据一律进 sidecar"里的 **uid 一项已被推翻**。原因是路径反查撑不住移动：
> 本地 `mv` 一个文件后，sync-state 的映射还指着旧路径，push 只能理解成"旧文档没了 +
> 新文档出现"，于是归档旧 memo、新建一个新 uid——版本历史、评论、reactions、分享链接
> 和所有指向它的引用全部留在归档件上。事后配对（内容哈希 / git rename）在
> "先改后移"和"先移后改"下都不成立，所以身份必须随文件走。
>
> 现在每篇文档在**末尾**带一行 `<!-- memogit-id: memos/<uid> -->`（VIEW 是 JSON 顶层
> 键）。本节真正的约束——**不往文件头部塞 memogit frontmatter**，第一个 `---` 块永远
> 属于用户的 Obsidian properties——完整保留。标记只存在于本地，push 前剥离，服务端
> content 永远不含它。详见 [[04-move-semantics-and-doc-identity]]。
